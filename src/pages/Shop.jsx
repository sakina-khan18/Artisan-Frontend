...
import { Filter, RefreshCw, X, Star, Sliders, Search, Heart ,Check, Store, Gift, Award, ArrowRight, Info} from 'lucide-react';
import { ShoppingBag, Scissors, Combine, Sparkles, Shirt, Diamond,ShoppingCart } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden aspect-square bg-gray-100 rounded-t-xl">
        ...
        <div className={`absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center transition-opacity duration-300 ${hovered ? 'opacity-100' : 'opacity-0'}`}>
          <button onClick={()=>{handleClick(product.id)}} className="bg-white text-amber-600 hover:bg-amber-50 px-4 py-2 rounded-lg font-medium text-sm shadow-md transition-all transform hover:scale-105">
            View Details
          </button>
        </div>
...
const Shop = () => {
  ...
  const filterAndSort = useCallback(() => {
    const { products, selectedCategory, priceRange, sortBy, searchQuery } = state;
  ...
  const FilterSection = memo(({ isMobile = false }) => (
    <div className={`bg-white rounded-xl shadow-sm ${isMobile ? 'p-5' : 'p-6'}`}>
      ...
      {isMobile && (
        <div className="flex gap-2">
          <button
            onClick={() => setState(prev => ({ ...prev, mobileFiltersOpen: false }))}
      ...
      <>
            ...
            >
              ...
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-medium text-gray-900 group-hover:text-amber-600 transition-colors">{product.title}</h3>
                ...
                <button className="mt-3 w-full bg-white border border-amber-600 text-amber-600 hover:bg-amber-600 hover:text-white py-2 rounded-lg transition-colors text-sm font-medium">
                  Add to Cart
                </button>
              </div>
            </motion.div>
        ...
        {pageCount > 1 && (
          <div className="mt-12 flex justify-center">
            <div className="flex gap-2">
              <button
                onClick={() => setState(prev => ({ ...prev, activePage: Math.max(1, prev.activePage - 1) }))}
              ...
              {Array.from({ length: pageCount }, (_, i) => (
                <button
                  key={i + 1}
                  onClick={() => setState(prev => ({ ...prev, activePage: i + 1 }))}
              ...
              <button
                onClick={() => setState(prev => ({ ...prev, activePage: Math.min(pageCount, prev.activePage + 1) }))}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
        <div className="flex flex-col md:flex-row items-center justify-between">
          <div className="mb-8 md:mb-0 md:mr-8 md:w-3/5">
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Artisan Marketplace</h1>
            <p className="text-amber-100 text-lg mb-6 max-w-lg">
              Discover unique handcrafted products from around the world, expertly made by skilled artisans.
            </p>
            <div className="flex flex-wrap gap-3">
              <button onClick={()=>{navigator("/cart")}} className="bg-white text-amber-600 hover:bg-amber-50 px-6 py-3 rounded-lg font-medium shadow-md transition-colors flex items-center">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <button
          onClick={() => setState(prev => ({ ...prev, mobileFiltersOpen: true }))}
          ...
          <div className="flex-1">
            ...
            >
              <p className="text-gray-600 mb-3 sm:mb-0">
                <span className="font-medium">{state.filteredProducts.length}</span> products
                {state.selectedCategory !== 'all' && (
                  <span> in <span className="text-amber-600 font-medium">{categories.find(c => c.id === state.selectedCategory)?.name}</span></span>
                )}
              </p>
      ...
      <div className="bg-amber-50 py-12 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-center mb-8">Featured Artisans</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {['Maya Crafts', 'Pottery Masters', 'Eco Designs', 'Silver Artisans'].map((artisan, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden border-2 border-amber-200">
                  <img src={`/api/placeholder/100/100`} alt={artisan} className="w-full h-full object-cover" />
                </div>
